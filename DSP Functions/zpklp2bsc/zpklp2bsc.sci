...
//Calling Sequences
//
//[Z2,P2,K2,AllpassNum,AllpassDen] = zpklp2bsc(Z,P,K,Wo,Wt) returns zeros, Z2, poles, P2, and gain factor, K2, of the target filter transformed from the real lowpass prototype by applying a first-order real lowpass to complex bandstop frequency transformation.It also returns the numerator, AllpassNum, and the denominator, AllpassDen, of the allpass mapping filter. The prototype lowpass filter is given with zeros, Z, poles, P, and gain factor, K.
//
//Input Parameters:
...
//                      p = roots(a);
//                      k = b(1);
//                      [z2,p2,k2] = zpklp2bsc(z,p,k,0.5,[0.25, 0.35]);
//
...
//Input Validaton Statement
    ...
    if argn(1)<1 | argn(1)>5 then
        error("Number of output arguments should be between 1-5");
    end
...
// Calculating the numerator and denominator for the mapping filter
    Wc = sum(Wt) / 2;
    bw = max(Wt) - min(Wt);
    al = cos(%pi*(Wo+bw/2)/2) / cos(%pi*(Wo-bw/2)/2);
    be = exp(-%pi*%i*Wc);
